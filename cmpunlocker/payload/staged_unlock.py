...
STAGE 1: PCIe Gen 2 Only (gen2.service at boot, lowest risk)
  - Write Gen 2 target to BAR0 (doesn't require PLM open)
  - Verify BAR0 write succeeds
  - Full power-off (cold shutdown, not a warm reboot) required: PCIe link
    speed is renegotiated during power-on link training, so a warm/soft
    reboot may leave the old link speed reported in lspci
...
STAGE 2: PLM Opening + Core + Feature Unlocks (Automatic via Daemon)
  ...
  - Write memory unlock (CFG1/LMR) for 40GB (firmware-locked maximum)
  - Write compute unlock (SS0/SS1) for 1410+ MHz
  - Apply all feature unlocks (PCIe Gen 3-5, NVLink, ECC, ARC)
  - Daemon auto-runs this stage after Stage 1's cold shutdown/boot
  - Purpose: Full unlock with all features

Stage 1 runs automatically at boot via gen2.service, which only marks the
stage complete if the Gen 2 BAR0 write actually verifies.
...
def stage1_pcie_gen2(pci_full: str) -> bool:
    ...
    try:
        with Bar0(pci_full) as bar0:
            gen2_addr = get('feature_unlocks.verified.pcie_gen2.addr')
            gen2_val = get('feature_unlocks.verified.pcie_gen2.value')
        ...
        if actual == gen2_val:
            ...
            log.info("[%s] NEXT: Cold shutdown (sudo shutdown -h now), power on, then verify: "
                     "lspci -s %s | grep Speed",
...
def stage2_plm_core_unlock(pci_full: str, gsp_path: str | None = None, target: str | None = None) -> bool:
    """Stage 2: PLM opening + core unlocks (memory + compute + features).

    After Stage 1's cold shutdown and power-on, this runs the full exploit
    to open PLM registers, write memory/compute unlock values, and apply
    all feature unlocks. Combines everything except Gen 2 (which Stage 1
    already applied).
    """
    log.info("[%s] === STAGE 2: PLM Opening + Full Core/Feature Unlocks ===", pci_full)

    if gsp_path is None:
        from cmpunlocker.payload.pipeline import _find_gsp
        gsp_path = _find_gsp()
    ...
    # Run the full exploit pipeline (PLM + memory target + compute + features)
    ok = run_full_unlock(pci_full, gsp_path, target)

    if ok:
        log.info("[%s] === STAGE 2 COMPLETE: Full Unlock Applied ===", pci_full)
        log.info("[%s] All unlocks applied: PLM open, %s memory, SM compute, features",
                  pci_full, target)
...
def run_next_stage(pci_full: str, gsp_path: str | None = None, target: str | None = None) -> bool:
